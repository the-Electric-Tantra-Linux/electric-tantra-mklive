...
ALL_ROOTFS=$(foreach arch,$(ARCHS),electric-tantra-$(arch)-ROOTFS-$(DATECODE).tar.xz)
ALL_PLATFORMFS=$(foreach platform,$(PLATFORMS),electric-tantra-$(platform)-PLATFORMFS-$(DATECODE).tar.xz)
ALL_SBC_IMAGES=$(foreach platform,$(SBC_IMGS),electric-tantra-$(platform)-$(DATECODE).img.xz)
ALL_CLOUD_IMAGES=$(foreach cloud,$(CLOUD_IMGS),electric-tantra-$(cloud)-$(DATECODE).tar.gz)
ALL_PXE_ARCHS=$(foreach arch,$(PXE_ARCHS),electric-tantra-$(arch)-NETBOOT-$(DATECODE).tar.gz)
...
dist: distdir-$(DATECODE)
	mv electric-tantra*$(DATECODE)* distdir-$(DATECODE)/
...
electric-tantra-%-ROOTFS-$(DATECODE).tar.xz: $(SCRIPTS)
...
electric-tantra-%-PLATFORMFS-$(DATECODE).tar.xz: $(SCRIPTS)
	$(SUDO) ./mkplatformfs.sh $(XBPS_REPOSITORY) -x $(COMPRESSOR_THREADS) $* electric-tantra-$(shell ./lib.sh platform2arch $*)-ROOTFS-$(DATECODE).tar.xz
...
electric-tantra-%-$(DATECODE).img.xz: electric-tantra-%-PLATFORMFS-$(DATECODE).tar.xz
	$(SUDO) ./mkimage.sh -x $(COMPRESSOR_THREADS) electric-tantra-$*-PLATFORMFS-$(DATECODE).tar.xz

# Some of the images MUST be compressed with gzip rather than xz, this
# rule services those images.
electric-tantra-%-$(DATECODE).tar.gz: electric-tantra-%-PLATFORMFS-$(DATECODE).tar.xz
	$(SUDO) ./mkimage.sh -x $(COMPRESSOR_THREADS) electric-tantra-$*-PLATFORMFS-$(DATECODE).tar.xz
...
electric-tantra-%-NETBOOT-$(DATECODE).tar.gz: $(SCRIPTS) electric-tantra-%-ROOTFS-$(DATECODE).tar.xz
	$(SUDO) ./mknet.sh electric-tantra-$*-ROOTFS-$(DATECODE).tar.xz
...
masterdir-%:
	$(SUDO) docker build --build-arg REPOSITORY=$(XBPS_REPOSITORY) --build-arg ARCH=$* -t electric-tantralinux/masterdir-$*:$(DATECODE) .
